...
export default function TaskCard({ task, index, onToggle, onDelete, onEdit, lang, deletingId, showArchive, onRestore }) {
  const isDeleting = task.id === deletingId;
  return (
    <div
      className={"task-card" + (task.completed ? " completed" : "") + (isDeleting ? " deleting" : "") + (task.archived ? " archived" : "")}
    ...
    >
      ...
      <div className="task-body">
        ...
        <div className="task-meta">
          {task.archived ? (
            <span className="reminder-badge archived">{t(lang, "archived")}</span>
          ) : task.completed ? (
      ...
      {showArchive ? (
        <button className="action-btn restore-btn" onClick={() => onRestore(task)} title="Restore">
